...
class TestKickFromGroup(BaseServerRestApi):
    ...
    def test_kicked_user_does_not_get_msg_notifications(self):
        ...
        self.assert_total_mqtt_sent_to(user_id=BaseTest.USER_ID, n_messages=0)        # sender doesn't increase
        self.assert_total_mqtt_sent_to(user_id=BaseTest.OTHER_USER_ID, n_messages=1)  # normal user increases
        self.assert_total_mqtt_sent_to(user_id=BaseTest.THIRD_USER_ID, n_messages=0)  # kicked user doesn't increase
