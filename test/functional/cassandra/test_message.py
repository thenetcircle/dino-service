...
class BaseMessageTest(BaseCassandraHandlerTest):
    ...
    def test_store_message(self) -> None:
        self.clear_messages()
        msg = self.handler.store_message(BaseMessageTest.GROUP_ID, BaseMessageTest.USER_ID,
            SendMessageQuery(message_payload=BaseMessageTest.MESSAGE_PAYLOAD, message_type=MessageTypes.MESSAGE))
        message = self.handler.get_all_messages_in_group(
            msg.group_id)
        ...
        self.assertEqual(1, len(message))
        message = self.handler.get_messages_in_group(
            msg.group_id, BaseMessageTest._generate_message_query(until=BaseMessageTest.LONG_AGO.timestamp()))
        self.assertEqual(0, len(message))
        # since does not work
        message = self.handler.get_messages_in_group(
        ...
        with self.assertRaises(NoSuchMessageException):
            self.handler.get_message_with_id(
                msg.group_id, msg.user_id, msg.message_id, msg.created_at.timestamp()-61)
        with self.assertRaises(NoSuchMessageException):
            self.handler.get_message_with_id(
                msg.group_id, msg.user_id, msg.message_id, msg.created_at.timestamp()+61)
        # invalid id
        ...
        with self.assertRaises(NoSuchMessageException):
            self.handler.get_message_with_id(
                msg.group_id, msg.user_id+1, msg.message_id, msg.created_at.timestamp())
        with self.assertRaises(NoSuchMessageException):
            self.handler.get_message_with_id(
        ...
        count = self.handler.count_messages_in_group_since(msg.group_id, since=BaseMessageTest.LONG_AGO)
        self.assertEqual(1, count)
        count = self.handler.count_messages_in_group_since(msg.group_id, since=utcnow_dt())
        ...
        count = self.handler.count_messages_in_group_from_user_since(
            msg.group_id, msg.user_id, utcnow_dt(), BaseMessageTest.LONG_AGO,
        )
        self.assertEqual(1, count)
        count = self.handler.count_messages_in_group_from_user_since(
            msg.group_id, msg.user_id, utcnow_dt(), utcnow_dt(),
        )
        self.assertEqual(0, count)
        count = self.handler.count_messages_in_group_from_user_since(
            msg.group_id, msg.user_id, utcnow_dt(), utcnow_dt(),
    ...
    def test_create_action_log(self) -> None:
        ...
        msg = self.handler.create_action_log(
            BaseMessageTest.USER_ID, BaseMessageTest.GROUP_ID,
        ...
        self.handler.edit_message(
            ...
            EditMessageQuery(created_at=msg.created_at.timestamp(), message_payload=new_payload)
        )
        # invalid range
        with self.assertRaises(NoSuchMessageException):
            self.handler.edit_message(
                msg.group_id, msg.user_id, msg.message_id,
                EditMessageQuery(created_at=BaseMessageTest.LONG_AGO.timestamp(), message_payload=new_payload)
            )
        # invalid id
        with self.assertRaises(NoSuchMessageException):
            self.handler.edit_message(
                str(uuid()), msg.user_id, msg.message_id,
                EditMessageQuery(created_at=msg.created_at.timestamp(), message_payload=new_payload)
            )
        with self.assertRaises(NoSuchMessageException):
            self.handler.edit_message(
                msg.group_id, msg.user_id+1, msg.message_id,
                EditMessageQuery(created_at=msg.created_at.timestamp(), message_payload=new_payload)
            )
        with self.assertRaises(NoSuchMessageException):
            self.handler.edit_message(
                msg.group_id, msg.user_id, str(uuid()),
                EditMessageQuery(created_at=msg.created_at.timestamp(), message_payload=new_payload)
            )
