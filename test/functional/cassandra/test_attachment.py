...
class BaseAttachmentTest(BaseCassandraHandlerTest):
    ...
    def insert_attachment(self) -> Tuple[MessageBase, MessageBase]:
        msg = self.handler.store_message(BaseAttachmentTest.GROUP_ID, BaseAttachmentTest.USER_ID,
            SendMessageQuery(message_payload=BaseAttachmentTest.MESSAGE_PAYLOAD, message_type=MessageTypes.IMAGE))
        atts = self.handler.store_attachment(BaseAttachmentTest.GROUP_ID, BaseAttachmentTest.USER_ID,
            msg.message_id, CreateAttachmentQuery(file_id=BaseAttachmentTest.FILE_ID,
                message_payload=BaseAttachmentTest.MESSAGE_PAYLOAD, created_at=utcnow_ts())
            )
        return msg, atts
    ...
    def test_storage_attachment(self) -> None:
        ...
        with self.assertRaises(NoSuchMessageException):
            self.handler.store_attachment(BaseAttachmentTest.GROUP_ID, BaseAttachmentTest.USER_ID,
                str(uuid()), CreateAttachmentQuery(file_id=BaseAttachmentTest.FILE_ID,
                    message_payload=BaseAttachmentTest.MESSAGE_PAYLOAD, created_at=utcnow_ts())
            )
        ...
        attachment = self.handler.get_attachment_from_file_id(
            atts.group_id, atts.created_at.replace(tzinfo=datetime.timezone.utc),
        ...
        count = self.handler.count_attachments_in_group_since(BaseAttachmentTest.GROUP_ID,
            BaseAttachmentTest.LONG_AGO)
        self.assertEqual(1, count)
        count = self.handler.count_attachments_in_group_since(BaseAttachmentTest.GROUP_ID,
            BaseAttachmentTest.LONG_AGO, atts.user_id)
        self.assertEqual(1, count)
        count = self.handler.count_attachments_in_group_since(BaseAttachmentTest.GROUP_ID,
            BaseAttachmentTest.LONG_AGO, atts.user_id+1)
        self.assertEqual(0, count)

        self.handler.delete_attachment(atts.group_id, arrow.get(atts_dt).shift(minutes=-1).datetime, DeleteAttachmentQuery(file_id=atts.file_id))
    ...
    def test_delete_attachment(self) -> None:
        ...
        group_to_atts = self.handler.delete_attachments_in_all_groups([], BaseAttachmentTest.USER_ID,
        ...
        group_to_atts = self.handler.delete_attachments_in_all_groups([(BaseAttachmentTest.GROUP_ID, utcnow_dt())],
        ...
        self.handler.delete_attachments_in_group_before(BaseAttachmentTest.GROUP_ID, BaseAttachmentTest.LONG_AGO)
        count = self.handler.count_attachments_in_group_since(BaseAttachmentTest.GROUP_ID,
            BaseAttachmentTest.LONG_AGO)
        self.assertEqual(3, count)
        self.handler.delete_attachments_in_group_before(BaseAttachmentTest.GROUP_ID, BaseAttachmentTest.LONG_AGO)
        count = self.handler.count_attachments_in_group_since(BaseAttachmentTest.GROUP_ID, utcnow_dt())
        self.assertEqual(0, count)
