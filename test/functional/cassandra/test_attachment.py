...
class BaseAttachmentTest(BaseCassandraHandlerTest):
    ...
    async def insert_attachment(self) -> Tuple[MessageBase, MessageBase]:
        ...
        atts = await self.handler.store_attachment(
            BaseAttachmentTest.GROUP_ID,
            BaseAttachmentTest.USER_ID,
            msg.message_id,
    ...
    async def test_storage_attachment(self) -> None:
        await self.clear_attachments()
        msg, atts = await self.insert_attachment()
        ...
        with self.assertRaises(NoSuchMessageException):
            await self.handler.store_attachment(
                BaseAttachmentTest.GROUP_ID,
                BaseAttachmentTest.USER_ID,
                str(uuid()),
                CreateAttachmentQuery(
        ...
        attachment = await self.handler.get_attachments_in_group_for_user(
            atts.group_id, user, BaseAttachmentTest._generate_message_query()
        )
        self.assertEqual(1, len(attachment))

        attachment = await self.handler.get_attachment_from_file_id(
            atts.group_id,
        ...
        with self.assertRaises(NoSuchAttachmentException):
            await self.handler.get_attachment_from_file_id(
                atts.group_id, atts_dt, AttachmentQuery(file_id=str(uuid()))
            )

        count = await self.handler.count_attachments_in_group_since(
        ...
        await self.handler.delete_attachment(
            atts.group_id,
    ...
    async def test_delete_attachment(self) -> None:
        await self.clear_attachments()
        for _ in range(3):
            await self.insert_attachment()
        count = await self.handler.count_attachments_in_group_since(
            BaseAttachmentTest.GROUP_ID, BaseAttachmentTest.LONG_AGO
        )
        self.assertEqual(3, count)

        group_to_atts = await self.handler.delete_attachments_in_all_groups(
