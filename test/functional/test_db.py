...
class TestDatabaseQueries(BaseDatabaseTest):
    def test_get_last_read_for_user_no_ugs(self):
        session = self.env.session_maker()
        group_id, last_read = self.env.db.get_last_read_for_user(BaseTest.USER_ID, session)
        self.assertIsNone(group_id)
        self.assertIsNone(last_read)

    def test_get_last_read_for_user_with_ugs(self):
        session = self.env.session_maker()
