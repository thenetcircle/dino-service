...
class TestPublicGroups(BaseServerRestApi):
    def test_created_at_set_to_x_days_ago(self):
        ...
        self.send_message_to_group_from(group_id, BaseTest.USER_ID, amount=half_max_count)
        self.user_joins_group(group_id, BaseTest.THIRD_USER_ID)

        # new user has joined, but not enough msgs, so delete_before should be max_days ago still
        self.user_joins_group(group_id, BaseTest.THIRD_USER_ID)
        user_stats = self.get_user_stats(group_id, BaseTest.THIRD_USER_ID)
        delete_before = to_dt(user_stats["stats"]["delete_before"])
        timedelta = utcnow_dt() - delete_before
        self.assertEqual(max_days, timedelta.days)

    def test_delete_before_set_to_x_messages_ago(self):
        ...
        self.send_message_to_group_from(group_id, BaseTest.USER_ID, amount=2 * max_count)
        histories = self.histories_for(group_id, BaseTest.USER_ID, per_page=500)
        ...
        self.send_message_to_group_from(group_id, BaseTest.THIRD_USER_ID, amount=max_count)
        histories = self.histories_for(group_id, BaseTest.THIRD_USER_ID, per_page=500)
        n_messages = len(histories["messages"])
        self.assertEqual(2 * max_count, n_messages)
