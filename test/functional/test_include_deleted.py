...
class TestIncludeDeletedMsgs(BaseServerRestApi):
    async def test_include_deleted_in_history(self):
        group_id = users_to_group_id(BaseTest.USER_ID, BaseTest.OTHER_USER_ID)
        creation_time = -1

        for _ in range(5):
            msg = await self.send_1v1_message()
            creation_time = msg["created_at"]

        self.assertEqual(5, len((await self.histories_for(group_id))["messages"]))
        ...
        self.assertEqual(0, len((await self.histories_for(group_id))["messages"]))
        ...
        self.assertEqual(10, len((await self.histories_for(
            group_id=group_id,
            admin=True,
