...
class TestSendAttachments(BaseServerRestApi):
    def assert_is_in_attachment_table(self, message_type: int):
        group_message = self.send_1v1_message(message_type=message_type)
        self.update_attachment(group_message["message_id"], group_message["created_at"])

        atts = self.attachments_for(group_message["group_id"])

        self.assertEqual(message_type, group_message["message_type"])
        self.assertEqual(message_type, atts[0]["message_type"])

    def test_send_image_is_in_attachment_list(self):
        self.assert_is_in_attachment_table(MessageTypes.IMAGE)

    def test_send_video_is_in_attachment_list(self):
        self.assert_is_in_attachment_table(MessageTypes.VIDEO)

    def test_send_audio_is_not_in_attachment_list(self):
        group_message = self.send_1v1_message(message_type=MessageTypes.AUDIO)
        self.update_attachment(group_message["message_id"], group_message["created_at"])
        ...
        msgs = self.histories_for(group_message["group_id"])
        self.assertEqual(1, len(msgs))
        self.assertEqual(MessageTypes.AUDIO, msgs["messages"][0]["message_type"])

    def test_delete_attachments_audio_ignored(self):
        group_message = self.send_1v1_message(message_type=MessageTypes.AUDIO)
        self.update_attachment(group_message["message_id"], group_message["created_at"])
        ...
        self.delete_attachments_in_all_groups(send_action_log_query=False)
    ...
    def test_delete_video(self):
        group_message = self.send_1v1_message(message_type=MessageTypes.VIDEO)
        self.update_attachment(group_message["message_id"], group_message["created_at"])
