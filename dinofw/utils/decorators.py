        ...
        def decorator(*a, **k):
            def _pre_process(*args, **kwargs):
                ...
                try:
                    ...
                    is_valid, status_code, message = getattr(
                        validation.request, validation_name
                    )(activity)
                    if is_valid:
                        all_ok = True
            ...
            try:
                ...
                if not exception_occurred:
                    environ.env.stats.timing(
                        "event." + validation_name, (time.time() - start) * 1000
                    )
            ...
            try:
                ...
                if should_disconnect and environ.env.config.get(
                    ...
                    eventlet.spawn_after(
                        seconds=1, func=_delayed_disconnect, sid=environ.env.request.sid
                    )
                return 500, str(e)
            finally:
                if tb is not None:
