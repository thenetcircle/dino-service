    ...
    def handle_disconnect(arg: tuple) -> None:
        ...
        def leave_private_room(user_id, current_sid):
            ...
            try:
                ...
                logger.debug(
                    'a user disconnected [id: "%s", name: "%s", sid: "%s"]'
                    % (user_id, user_name, current_sid)
                )
                ...
                try:
                    ...
                    logger.warning(
                        "could not remove sid {} for user {}: {}".format(
                            current_sid, user_id, str(e)
                        )
                    )
                ...
                if len(all_sids) == 0 or (
                    current_sid in all_sids and len(all_sids) == 1
                ):
                    environ.env.leave_room(user_id)
                    environ.env.db.reset_sids_for_user(user_id)
                    for key in SessionKeys.temporary_keys.value:
                        environ.env.auth.update_session_for_key(
                            activity.actor.id, key, False
                        )
        ...
        def leave_all_public_rooms_and_emit_leave_events(user_id, current_sid):
            try:
                ...
                logger.debug(
                    "request for failed leave_all_public_rooms_and_emit_leave_events(): %s"
                    % str(data)
                )
                logger.exception(traceback.format_exc())
                environ.env.capture_exception(sys.exc_info())
        ...
        def emit_disconnect_event(user_id, current_sid) -> None:
            try:
                ...
                if user_id is None or user_id == "None":
                    logger.warning(
                        "blank user_id on disconnect event, trying sid instead"
                    )
                    ...
                    if (
                        ...
                        logger.error(
                            "blank sid as well as blank user id, ignoring disconnect event"
                        )
                        return
                    ...
                    try:
                        ...
                        logger.warning(
                            'could not get user id from sid "{}": {}'.format(
                                current_sid, str(e)
                            )
                        )
                        environ.env.capture_exception(sys.exc_info())
                ...
                if user_id != "-1":
                    sid_ended_event = utils.activity_for_sid_disconnect(
                        user_id, user_name, current_sid
                    )
                ...
                logger.debug(
                    "request for failed emit_disconnect_event(): %s" % str(data)
                )
                logger.exception(traceback.format_exc())
                environ.env.capture_exception(sys.exc_info())
