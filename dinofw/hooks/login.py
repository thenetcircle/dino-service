...
class OnLoginHooks(object):
    ...
    def update_session_and_join_private_room(arg: tuple) -> None:
        ...
        environ.env.session[SessionKeys.user_agent_browser.value] = (
            user_agent_browser or ""
        )
        environ.env.session[SessionKeys.user_agent_version.value] = (
            user_agent_version or ""
        )
        environ.env.session[SessionKeys.user_agent_platform.value] = (
            user_agent_platform or ""
        )
        environ.env.session[SessionKeys.user_agent_language.value] = (
            user_agent_language or ""
        )

        user_info = {
            SessionKeys.avatar.value: environ.env.session.get(SessionKeys.avatar.value)
            or "",
            SessionKeys.app_avatar.value: environ.env.session.get(
                SessionKeys.app_avatar.value
            )
            or "",
            SessionKeys.app_avatar_safe.value: environ.env.session.get(
                SessionKeys.app_avatar_safe.value
            )
            or "",
            SessionKeys.age.value: environ.env.session.get(SessionKeys.age.value) or "",
            SessionKeys.gender.value: environ.env.session.get(SessionKeys.gender.value)
            or "",
            SessionKeys.membership.value: environ.env.session.get(
                SessionKeys.membership.value
            )
            or "",
            SessionKeys.group.value: environ.env.session.get(SessionKeys.group.value)
            or "",
            SessionKeys.country.value: environ.env.session.get(
                SessionKeys.country.value
            )
            or "",
            SessionKeys.has_webcam.value: environ.env.session.get(
                SessionKeys.has_webcam.value
            )
            or "",
            SessionKeys.fake_checked.value: environ.env.session.get(
                SessionKeys.fake_checked.value
            )
            or "",
            SessionKeys.is_streaming.value: environ.env.session.get(
                SessionKeys.is_streaming.value
            )
            or "",
            SessionKeys.enabled_safe.value: environ.env.session.get(
                SessionKeys.enabled_safe.value
            )
            or "",
    ...
    def autojoin_rooms(arg: tuple) -> None:
        ...
        if not str(
            environ.env.config.get(ConfigKeys.AUTOJOIN_ENABLED, "false")
        ...
        for room_id, acls in room_acls.items():
            ...
            join_data = data.copy()
            join_data["target"] = {"id": room_id, "objectType": "room"}

            is_valid, error_msg = validation.acl.validate_acl_for_action(
                activitystreams.parse(join_data), ApiTargets.ROOM, ApiActions.JOIN, acls
            )
            ...
            join_data = ActivityBuilder.enrich(
                {
                    "verb": "join",
                    "actor": {"id": activity.actor.id},
                    "target": {"id": room_id},
                }
            )
            environ.env.observer.emit(
                "on_join", (join_data, activitystreams.parse(join_data))
            )
