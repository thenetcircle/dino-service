...
async def group_get_message(group_id: str, user_id: int, message_id: str) -> Message:
...
async def group_users(group_id: str, query: PaginationQuery) -> GroupUsers:
    return await environ.env.rest.user.users(group_id, query)


@app.get("/v1/userstats/{user_id}", response_model=UserStats)
async def user_stats(user_id: int) -> UserStats:
...
async def user_group_stats(group_id: str, user_id: int) -> UserGroupStats:
